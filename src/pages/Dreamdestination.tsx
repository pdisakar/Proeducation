        ...
        <div className="destinations mt-8">
          <ul className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {countries.map((country) => (
              <li
                key={country.id}
                className="text-center flex items-center justify-center flex-col px-6 py-6 border border-muted/20 rounded-[4px]"
              >
                <svg className="icon" width="120" height="75">
                  <use
                    xlinkHref={`/icons.svg#${country.id}`}
                    fill="currentColor"
                  ></use>
                </svg>
                <p className=" mt-3 text-lg text-headings font-semibold leading-[18px] ">
                  {country.name}
                </p>
              </li>
            ))}
